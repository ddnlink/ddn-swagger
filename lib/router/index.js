...
function compareRouteEntries(a, b) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    // 关键修正：如果都是静态路径，按字母顺序排序，确保同一前缀的路由聚合在一起（解决 /dao 和 /daos 混杂的问题）
    if (typeA === 0) {
      return sA.localeCompare(sB);
    }

    // 如果是参数或通配符，按定义顺序排序（保留冲突解决的优先级）
    return (a._index || 0) - (b._index || 0);
  }
 ...
 */
function isRouteRegistered(router, method, route) {
  const normalizedRoute = normalizeRoutePath(route);
  return (router.stack || []).some(layer => {
    // layer.path 在某些 koa-router 版本中可能不存在，改用 layer regexp 兜底
    if (!layer.path) return false;
    return layer.methods &&
      layer.methods.some(m => m.toLowerCase() === method.toLowerCase()) &&
      layer.path === normalizedRoute;
  });
}
...
module.exports = {
  ...
  basicRouterRegister: app => {
    ...
    let swaggerH5;
    try {
      // 使用 require.resolve 定位锚点文件，兼容性更好
      const anchor = require.resolve('../../app/public/swagger-ui-bundle.js');
      swaggerH5 = path.dirname(anchor);
    } catch (e) {
      // 回退方案
      swaggerH5 = path.join(__dirname, '../../app/public');
      app.logger.warn(`[@ddn/swagger-docs] Failed to resolve swagger-ui path: ${e.message}`);
    }

    if (fs.existsSync(swaggerH5)) {
      app.use(staticCache(swaggerH5, {}, {}));
      app.logger.info(`[@ddn/swagger-docs] register router: get /swagger-ui.html from ${swaggerH5}`);
    } else {
      app.logger.error(`[@ddn/swagger-docs] Swagger UI directory not found: ${swaggerH5}`);
    }
  ...
  RouterRegister: app => {
    ...
    for (const entry of routeEntries) {
      ...
      if (instance.prototype) {
        const control = convertControllerPath(instance.prototype.pathName, controller, app.logger);
      ...
      // Plan C: 检查路由是否已被手动注册（如 app/router.js），避免重复
      if (isRouteRegistered(router, req.method, route)) {
        if (config.routerCli) {
          app.logger.info(`[@ddn/swagger-docs] skip already registered route: ${req.method} ${req.route} for ${direct.replace(path.sep, '-')}-${req.func}`);
        }
...
module.exports.isRouteRegistered = isRouteRegistered;
