...
function buildDocument(app) {
  ...
  // 递归获取 tags&paths
  tag_path = getTag_Path(filepath, securitys, config, components, app);
...
// 示例用法
function getTag_Path(fileDir, securitys, config, components, app) {
  ...
  for (let name of names) {
    ...
    if (stat.isDirectory()) {
      const subPath = getTag_Path(filepath, securitys, config, components, app);
      // 合并子目录的扫描结果
    ...
    if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(name)) !== -1) {
      ...
      if (blocks.length === 0 || !hasController(blocks[0].comment)) continue;

      // 当前注释块集合的所属tag-group, 并添加至swagger.tags中
      let controller = comment.getComment(blocks[0].comment, _.CONTROLLER)[0];
      ...
      for (let i = 1; i < blocks.length; i++) {
        try {
          if (isIgnore(blocks[i].comment)) continue;

          // 解析路由
          let routers = comment.getComment(blocks[i].comment, _.ROUTER);

          // 如果没有@router注解，跳过这个注释块
          if (!routers || routers === false) {
            continue;
          }

          // 检查路由格式是否正确
          if (!routers[0] || !routers[0][1] || !routers[0][2]) {
            app.logger.error(`[@ddn/swagger-docs] 路由格式错误 in ${filepath}:`, {
              routers,
              blockIndex: i,
              blockContent: blocks[i].comment.substring(0, 200) + '...',
            });
            continue;
          }
          ...
          if (routers) {
            ...
            path_method.summary = generateSummary(blocks[i].comment);
            path_method.description = generateDescription(blocks[i].comment);
            ...
            const requests = generateRequest(blocks[i].comment, routers, components, config);
            ...
            path_method.security = generateSecurity(blocks[i].comment, securitys, config, app);
            path_method.responses = generateResponse(blocks[i].comment, routers, components);
            path_method.deprecated = isDeprecated(blocks[i].comment);
            ...
            // 绑定route和function
            let contractName = getContractInBody(blocks[i].comment, components);
            let security = generateSecurity(blocks[i].comment, securitys, config, app);

            // Use the function name parsed from the file content
            let funcName = blocks[i].funcName;
            if (!funcName) {
              app.logger.warn(`[@ddn/swagger-docs] Warning: Could not determine function name for route ${method} ${route} in ${filepath}. Please ensure the function definition follows the JSDoc comment.`);
              // Fallback or skip? If we skip, the route won't work.
              continue;
            }

            // 诊断日志：routerCli 开启时打印每个路由的 funcName 匹配状态
            if (config.routerCli) {
              app.logger.info(`[@ddn/swagger-docs] Parsed route: ${method} ${route} -> ${funcName} in ${filepath.replace(app.config.baseDir, '')}`);
            }

            // 当 enableSecurity=false 但存在安全注解时发出警告
            if (!config.enableSecurity && security && security.length > 0) {
              app.logger.warn(`[@ddn/swagger-docs] Security annotations detected for ${method} ${route} but enableSecurity is false. These will be ignored in the generated spec.`);
            }
          ...
          app.logger.error(`[@ddn/swagger-docs] FATAL ERROR: Failed to parse router definition in ${filepath}:`);
          app.logger.error(`  - Error: ${err.message}`);
          // 严格模式下抛出异常
          if (config.strictMode) {
            throw err;
          }
          app.logger.warn('  - This specific route will be skipped, but other routes will still be registered.');
          continue;
        }
      }

      // 对路由进行排序，将包含路径参数的路由放在最后
      bundler.routers.sort((a, b) => {
        const aHasParam = a.route.includes('/{') || a.route.includes('/:');
        const bHasParam = b.route.includes('/{') || b.route.includes('/:');

        // 如果一个有参数一个没有，没有参数的排在前面
        if (aHasParam && !bHasParam) return 1;
        if (!aHasParam && bHasParam) return -1;

        // 如果都有参数或都没有参数，按路由长度排序（更具体的路由在前）
        if (aHasParam && bHasParam) {
          // 计算路径段数量，更多段的更具体
          const aSegments = a.route.split('/').length;
          const bSegments = b.route.split('/').length;
          if (aSegments !== bSegments) {
            return bSegments - aSegments; // 更多段的在前
          }

          // 如果段数相同，计算非参数段的数量
          const aNonParamSegments = a.route.split('/').filter(seg => !seg.includes('{') && !seg.includes(':')).length;
          const bNonParamSegments = b.route.split('/').filter(seg => !seg.includes('{') && !seg.includes(':')).length;
          return bNonParamSegments - aNonParamSegments; // 更多非参数段的在前
        }
 ...
 */
function generateSecurity(block, securitys, config, app) {
  let securityDoc = [];
  for (let security of securitys) {
    if (block.indexOf(`@${security}`) > -1) {
      let securityItem = {};
      const securityScheme = config.securitySchemes[security];

      if (securityScheme.type === 'apiKey') {
        securityItem[security] = [];
        securityItem[security].push(securityScheme);
      } else if (securityScheme.type === 'oauth2') {
        securityItem[security] = [];
        Object.keys(securityScheme.scopes).forEach(i => {
          securityItem[security].push(i);
        });
      } else if (securityScheme.type === 'http') {
        // 支持 HTTP 认证方案（Basic, Bearer 等）
        securityItem[security] = [];
        securityItem[security].push(securityScheme);
      } else {
        // 支持自定义认证类型，将其视为 apiKey 类型处理
        if (app) app.logger.warn(`[@ddn/swagger-docs] 检测到自定义认证类型: ${securityScheme.type}，将作为 apiKey 类型处理`);
        securityItem[security] = [];
        // 创建标准化的 apiKey 格式
        const normalizedScheme = {
          ...
          description: securityScheme.description || `自定义认证: ${securityScheme.type}`,
        };
        securityItem[security].push(normalizedScheme);
      }
    ...
    for (let response of responses) {
      ...
      if (response[2]) {
        if (!components.hasOwnProperty(response[2])) {
          // 提供更详细的错误信息，包括可用的类型列表
          const availableTypes = Object.keys(components).slice(0, 10).join(', ');
          const totalTypes = Object.keys(components).length;
          const moreTypesInfo = totalTypes > 10 ? ` (and ${totalTypes - 10} more)` : '';

          throw new Error(`[@ddn/swagger-docs] Response type "${response[2]}" not found in Contract definitions for ${routers[0][1].toLowerCase()}:${routers[0][2]}. Available types: ${availableTypes}${moreTypesInfo}. Please check your Contract files in app/contract/ directory.`);
        }
  ...
  if (requestIn.toLowerCase() === 'body' && !_.itemType.includes(request[2])) {
    ...
    if (!request[2].startsWith('array')) {
      if (!components.hasOwnProperty(request[2])) {
        ...
        throw new Error(`[@ddn/swagger-docs] Request type "${request[2]}" not found in Contract definitions for ${routers[0][1].toLowerCase()}:${routers[0][2]}. Available types: ${availableTypes}${moreTypesInfo}. Please check your Contract files in app/contract/ directory.`);
      }
      ...
      if (ObjectType.includes(itemsType)) {
        ...
        if (!components.hasOwnProperty(itemsType)) {
          throw new Error(`[@ddn/swagger-docs] error at ${routers[0][1].toLowerCase()}:${routers[0][2]} ,the type of request parameter does not exit`);
        }
