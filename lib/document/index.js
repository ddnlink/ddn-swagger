...
function getTag_Path(fileDir, securitys, config, components) {
  ...
  for (let name of names) {
    ...
    if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(name)) !== -1) {
      ...
      // 获取所有的有效方法
      // let func = generateAPIFunc(filepath); // No longer needed for mapping, but maybe useful for validation?
      let bundler = {
        filePath: filepath,
        routers: [],
      };
      ...
      for (let i = 1; i < blocks.length; i++) {
        try {
          ...
          if (!routers[0] || !routers[0][1] || !routers[0][2]) {
            console.error(`[@ddn/swagger-docs] 路由格式错误 in ${filepath}:`, {
              routers,
              blockIndex: i,
          ...
          if (routers) {
            ...
            path_method.security = generateSecurity(blocks[i].comment, securitys, config);
            ...
            let security = generateSecurity(blocks[i].comment, securitys, config);
            
            // Use the function name parsed from the file content
            let funcName = blocks[i].funcName;
            if (!funcName) {
              console.warn(`[@ddn/swagger-docs] Warning: Could not determine function name for route ${method} ${route} in ${filepath}. Please ensure the function definition follows the JSDoc comment.`);
          ...
          console.error(`\n[@ddn/swagger-docs] FATAL ERROR: Failed to parse router definition in ${filepath}:`);
          console.error(`  - Error: ${err.message}`);
          console.error(`  - This specific route will be skipped, but other routes will still be registered.\n`);
          continue;
        }
      }
  ...
  for (let security of securitys) {
    if (block.indexOf(`@${security}`) > -1) {
      ...
      if (securityScheme.type === 'apiKey') {
        ...
        console.warn(`[ddn-swagger] 检测到自定义认证类型: ${securityScheme.type}，将作为 apiKey 类型处理`);
        securityItem[security] = [];
        // 创建标准化的 apiKey 格式
