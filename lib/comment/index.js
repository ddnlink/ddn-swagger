  ...
  generateCommentBlocks: filePath => {
    ...
    while ((match = block_regex.exec(fileString)) !== null) {
      ...
      if (comment.indexOf('ontroller') > -1 || comment.indexOf('outer') > -1 || comment.indexOf('gnore') > -1) {
        ...
        // (?:(?:static|async|get|set|export)\s+)*  # Zero or more modifiers (static async get set export)
        // (?:async\s+)?                            # Optional async (backward compat)
        // (?:function\s+)?                         # Optional function keyword
        // ([a-zA-Z0-9_$]+)                         # Capture method name
        // \s*                                      # Optional whitespace
        // (?:\(|=\s*(?:async\s*)?\(|:)             # Followed by (, or = (, or = async (, or : (for TS types?)

        const funcRegex = /^\s*(?:(?:static|async|get|set|export)\s+)*(?:async\s+)?(?:function\s+)?([a-zA-Z0-9_$]+)\s*(?:\(|=\s*(?:async\s*)?\(|:)/;
        ...
        if (funcMatch) {
          ...
          // Plan A fallback: permissive match for edge cases (decorators, complex modifiers, etc.)
          // Matches the last identifier before ( — handles patterns the primary regex misses
          const fallbackRegex = /^\s*(?:[a-zA-Z_$][\w$]*\s+)*([a-zA-Z_$][\w$]*)\s*\(/;
          const fallbackMatch = nextText.match(fallbackRegex);
          if (fallbackMatch) {
            funcName = fallbackMatch[1];
          }
        }
