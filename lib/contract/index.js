      ...
      if (!type.includes(def[field].type)) {
        ...
        if (source[def[field].type] && source[def[field].type].content) {
          ...
          if (def[field].type !== 'object') {
            if (APP) APP.logger.warn(`[@ddn/swagger-docs] Warning: Referenced type "${def[field].type}" not found for "${object}.${field}". Using 'object' type instead.`);
          }
        ...
        if (!itemType.includes(def[field].itemType)) {
          ...
          if (source[def[field].itemType] && source[def[field].itemType].content) {
            ...
            if (def[field].itemType !== 'object') {
              if (APP) APP.logger.warn(`[@ddn/swagger-docs] Warning: Referenced itemType "${def[field].itemType}" not found for "${object}.${field}". Using 'object' type instead.`);
            }
...
function buildValidateRule(source) {
  ...
  for (let object in source) {
    ...
    if (!source[object] || !source[object].content) {
      if (APP) APP.logger.warn(`[@ddn/swagger-docs] Warning: Invalid contract definition for "${object}". Skipping.`);
      continue; // 跳过无效的定义
    }
    ...
    for (let field in def) {
      // 检查 def[field] 是否存在
      if (!def[field]) {
        if (APP) APP.logger.warn(`[@ddn/swagger-docs] Warning: Invalid field definition for "${object}.${field}". Skipping.`);
        continue; // 跳过无效的字段定义
      }
      ...
      if (def[field].type === 'array') {
        if (!itemType.includes(def[field].itemType)) {
          ...
          if (!referencedItemType || !referencedItemType.content) {
            // 只有在真正找不到类型定义时才显示警告
            if (def[field].itemType !== 'object') {
              if (APP) APP.logger.warn(`[@ddn/swagger-docs] Warning: Referenced itemType "${def[field].itemType}" not found for "${object}.${field}". Using empty object rule instead.`);
            }
            def[field]['rule'] = {}; // 使用空对象作为默认规则
          } else {
