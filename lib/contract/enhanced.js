 ...
 */
let CONTRACT;
let APP; // Egg application instance for logging
...
function generateContract(app) {
  ...
  if (!fs.existsSync(contractPath)) {
    if (APP) APP.logger.warn(`[@ddn/swagger-docs] Contract path not found: ${contractPath}`);
    return;
  }

  loadContractFiles(contractPath);
  if (APP) APP.logger.info(`[@ddn/swagger-docs] Loaded ${Object.keys(CONTRACT).length} contract definitions: ${Object.keys(CONTRACT).slice(0, 10).join(', ')}`);
}
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (APP) APP.logger.error(`[@ddn/swagger-docs] Error loading contract file ${filepath}: ${error.message}`);
      }
    }
  }
}
...
function getProcessOrder(source) {
  ...
  function visit(typeName) {
    ...
    if (visiting.has(typeName)) {
      if (APP) APP.logger.warn(`[@ddn/swagger-docs] Circular dependency detected: ${typeName}`);
      return;
    }
...
function mergeExtendedType(target, parentType, source) {
  ...
  if (!parent || !parent.content) {
    if (APP) APP.logger.warn(`[@ddn/swagger-docs] Parent type not found: ${parentType}`);
    return;
  }
...
function processFieldType(fieldDef, source, objectName, fieldName, filePath) {
  ...
  if (!type.includes(result.type)) {
    if (source[result.type] && source[result.type].content) {
      ...
      if (APP) APP.logger.warn(`[@ddn/swagger-docs] Warning: Referenced type "${result.type}" not found for "${objectName}.${fieldName}". Using 'object' type instead.`);
      result.type = 'object';
    }
  }
  ...
  if (result.type === 'array') {
    ...
    if (!itemType.includes(result.itemType)) {
      if (source[result.itemType] && source[result.itemType].content) {
        ...
        if (APP) APP.logger.warn(`[@ddn/swagger-docs] Warning: Referenced itemType "${result.itemType}" not found for "${objectName}.${fieldName}". Using 'object' type instead.`);
